...
{% endraw %}
observe that $$sup_{j \geq n} h(j) \leq c$$ is the same as saying that for all values of $$h(j)$$ on the set $$\{j \geq n\}$$ are less than $$c$$, since if the $$sup$$ is less than $$c$$ all values are. Finally, we arrive at the statement,there exist positve constants $$N$$ and $$c$$ such that for all $$n \geq N$$,
{% raw %}
$$h(n) \leq c$$
{% endraw %}
