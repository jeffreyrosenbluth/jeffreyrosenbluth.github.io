...
$$
{% endraw %}
This means that there must be a $$c$$ and an $$N$$ such that, for all $$n \geq N$$
{% raw %}
$$
...
## big-$$\Omega$$ and little-$$\omega$$ and big-$$\Theta$$
If we think of *big-O* as being the asymptotic upper bound, then *big-*$$\Omega$$ is the asymptotic lower bound. It has the same definition as *big-O*, only the less than or equals becomes greater than or equals. A function $$f(n)$$ is $$\Omega(g(n))$$ if and only if $$g(n)$$ is $$O(f(n))$$. So we can use all of the machinery we built above to check if some function is *big-*$$\Omega$$ of some other function. The same relationship is true of *little-o* and *little-*$$\omega$$. A function $f$ that is both $$O(g)$$ and $$\Omega(g)$$ is said to be $$\Theta(g)$$. In other words $$f$$ is asymptotically equal to $$g$$.
